...
APP_ERROR readImage(std::string imgPath, MxBase::Image& image, ImageProcessor& imageProcessor)
{
    ...
    ret = imageProcessor.Decode(dataPtr, dataSize, image, ImageFormat::YUV_SP_420);
    // endif
#endif
    if (ret != APP_ERR_OK)
    {
...
void poptProcess(std::vector<MxBase::Tensor> modelOutputs, std::shared_ptr<MxBase::Yolov3PostProcess> postProcessorDptr, MxBase::Size originalSize, MxBase::Size resizeSize)
{
    ...
    float resizeRate = originalSize.width > originalSize.height ? (originalSize.width / 640.0) : (originalSize.height / 640.0);
...
APP_ERROR main(int argc, char *argv[])
{
    ...
    // model init
    MxBase::Model yoloModel(modelPath, deviceId);
    // postprocessor init
    ...
    MxBase::Size resizeSize = MxBase::Size(YOLOV5_RESIZE, YOLOV5_RESIZE);
    ...
    // 后处理
    poptProcess(modelOutputs, postProcessorDptr, originalSize, resizeSize);
