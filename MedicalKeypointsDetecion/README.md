...
具体步骤如下：
**步骤1** 获取模型onnx文件
，下载链接为https://mindx.sdk.obs.myhuaweicloud.com/ascend_community_projects/body_Keypoints_detection/pose_model.zip
...
【注】本模型定位场景为医学应用下的人体关键点检测，故只针对特定体态进行识别，建议从提供的测试图片或根据对应体态自行拍摄图片进行测试。供开发者参考的测试图片(一共23张图片，对应23个常见体位)链接如下：https://mindx.sdk.obs.cn-north-4.myhuaweicloud.com/ascend_community_projects/body_Keypoints_detection/phto_for_Developers.zip
...
**步骤1** 获取测试集的图片,确保测试集的输入图片为jpg格式。获取测试集的标注文件（`person_keypoints_val2017.json`）。确保其存放于data文件夹下。（数据集暂未开源）

**步骤2** 修改evaluate.py中的一些路径：

```
IMAGEFOLDER = ' ' #修改为测试集的路径
FILE_ANNO = ' ' #修改为测试集的标注文件路径
TXT = '' #体位分类情况
```

**步骤3** 确保已安装好pycocotools库，找到库文件cocoeval.py所在位置，修改默认值。

在setKpParams函数中，修改self.kpt_oks_sigmas的值：
