...
在整个测试集上进行性能测试，未进行模型1的推理时间统计（因为该模型来自华为仓内，未进行任何调整，无需加入性能测试），模型2(hrnet-w48)的推理总花费时间为116s，fps=3651/116=31.47.原modelzoo下的hrnet-w48在五百张图片上推理耗时551s，fps=1.1.可见本模型在模型上的优化可以很好的提升性能。
