...
剪枝完成后的模型使用MindX SDK开发部署，在昇腾芯片上进行检测与评估，可将结果进行可视化并保存。主要处理流程为：输入图片->图片解码->图片缩放->模型推理->模型后处理->输出推理结果。
...
在编译运行项目前，需要设置环境变量： 1、根据自己安装的ascend-toolkit下的set_env.sh以及sdk下的set_env.sh设置环境变量。2、通过命令把全局日志级别设置为error级别 ''' export ASCEND_GLOBAL_LOG_LEVEL=3 '''
...
执行该命令后会在当前文件夹下生成项目需要的模型文件 `prune55.om`。执行后终端输出为：
