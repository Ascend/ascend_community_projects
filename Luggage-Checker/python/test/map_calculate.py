...
MINOVERLAP = [round(x, 2) for x in np.arange(0.5, 1, 0.05)] # default value (defined in the PASCAL VOC2012 challenge)
...
def calculate(sum_ap, fp, tp, counter_per_class, class_name):
    """
...
def cal_average_pricision(output_file, gt_classes, labels,
                        class_bbox, counter_per_class, minooverlap):
    """
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        ret = calculate(sum_average_pricision, fp, tp, counter_per_class, class_name)
        sum_average_pricision = ret.get('sumAp')
        text = ret.get('text')
        prec = ret.get('prec')
        rec = ret.get('rec')
        print(text)
    ...
    mean_ap = sum_average_pricision / n_classes
    text = "MeanAp = {0:.2f}%".format(mean_ap * 100)
    print(text)
    return mean_ap
...
if __name__ == '__main__':
    ...
    parser.add_argument('--npu_txt_path', default="./test_nopre_post", help='the path of the predict result')
    parser.add_argument('--output_file', default="./output.txt", help='save result file')
    parser.add_argument('--threshold', default=0.01, help='threshold of the object score')

    arg = parser.parse_args()
    arg = check_args(arg)
    SUM_AP = 0
    for minolap in MINOVERLAP:
        ...
        SUM_AP += cal_average_pricision(arg.output_file,
                                                   get_classes,
                                                   gt_file_bbox,
                                                   predict_bbox,
                                                   count_per_class,
                                                   minolap)
    print("Total:{}".format(SUM_AP / 10))
