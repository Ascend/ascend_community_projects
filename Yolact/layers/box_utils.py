...
def intersect(box_a, box_b):
    ...
    AA = np.broadcast_to(AA,[n,A,B,2])
    BB = box_b[:, :, 2:]
    BB = np.expand_dims(BB, axis=1)
    BB = np.broadcast_to(BB,[n,A,B,2])
    max_xy = np.minimum(AA, BB)
    min_xy = np.maximum(AA, BB)
    out = np.clip(max_xy - min_xy, a_min=0, a_max=np.float('inf'))
    out = out.prod(axis=3)
    return out
#
def jaccard(box_a, box_b, iscrowd:bool=False):
    ...
    area_a = np.expand_dims(area_a, axis=2)
    area_a = np.broadcast_to(area_a, inter.shape)
    area_b = ((box_b[:, :, 2]-box_b[:, :, 0]) * (box_b[:, :, 3]-box_b[:, :, 1])) 
    area_b = np.expand_dims(area_b, axis=1)
    area_b = np.broadcast_to(area_b, inter.shape)
    union = area_a + area_b - inter

    out = inter / area_a if iscrowd else inter / union
    return out if use_batch else np.expand_dims(out, axis=0)
...
def mask_iou(masks_a, masks_b, iscrowd=False):
    ...
    area_a = masks_a.sum(axis=1)
    area_a = np.expand_dims(area_a, axis = 1)
    area_b = masks_b.sum(axis=1)
    area_a = np.expand_dims(area_a, axis = 0)
...
def sanitize_coordinates(_x1, _x2, img_size:int, padding:int=0, cast:bool=True):
    ...
    if cast:
        _x1 = _x1.long()
        _x2 = _x2.long()
    x1 = np.empty(_x1.shape)
    x2 = np.empty(_x2.shape)
    ...
    for i in range(len(_x2)):   
        if _x1[i]> _x2[i]:
            ...
            x2[i] = _x2[i]
    x1 = np.clip(x1-padding, a_min = 0, a_max = float('inf'))
    x2 = np.clip(x2+padding, a_min = -float('inf'), a_max = img_size)
    return x1, x2
...
def crop(masks, boxes, padding:int=1):
    ...
    cols = np.arange(w,dtype=x1.dtype).reshape(1,-1,1)
