...
sys.path.insert(0, '/home/weixing2/yolact-master/infer/sdk/utils')
import pyximport
...
class Detect(object):
    ...
    def __call__(self, predictions):
        ...
        with timer.env('Detect'):
            ...
            x= conf_data.reshape(batch_size, num_priors, self.num_classes)
            conf_preds = conf_data.reshape(batch_size, num_priors, self.num_classes).transpose(0, 2, 1)
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        ###zzzzzzzz#####conf_scores, _ = torch.max(cur_scores, dim=0)
        conf_scores = np.max(cur_scores, 0)
...
#        cos_max = np.max(cos_similarity, 0)
...
#
    def traditional_nms(self, boxes, masks, scores, iou_threshold=0.5, conf_thresh=0.05):
        ...
        scores = abs(np.sort(-scores,axis=0))
        idx2 = abs(np.argsort(-scores,axis=0))
