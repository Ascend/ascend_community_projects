...
M1 = (stereoParams.CameraParameters1.IntrinsicMatrix).'; % 右相机内参矩阵，需要转置为左乘的矩阵
...
D1 = zeros(1,5); 
D1(1:5) = [RadialDistortion1(1:2), TangentialDistortion1, RadialDistortion1(3)]; %构成opencv中的畸变系数向量[k1,k2,p1,p2,k3]

M2 = (stereoParams.CameraParameters2.IntrinsicMatrix).'; % 右相机内参矩阵，需要转置为左乘的矩阵
...
D2 = zeros(1,5);
D2(1:5) = [RadialDistortion2(1:2), TangentialDistortion2, RadialDistortion2(3)]; %构成opencv中的畸变系数向量[k1,k2,p1,p2,k3]
...
for i=1:size(A,1)
    for j=1:size(A,2)
        data.appendChild(doc.createTextNode(sprintf('%.4f ',A(i,j))));
    end
