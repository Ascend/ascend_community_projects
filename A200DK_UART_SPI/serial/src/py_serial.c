...
static PyObject *Serial_input_waiting(SerialObject *self, PyObject *args) {
    ...
    result = (serial_input_waiting(self->fd, &count) < 0) ? Py_True : Py_False;
...
static PyObject *Serial_output_waiting(SerialObject *self, PyObject *args) {
    ...
    result = (serial_output_waiting(self->fd, &count) < 0) ? Py_True : Py_False;
...
static PyObject *Serial_poll(SerialObject *self, PyObject *args) {
    ...
    result = (serial_poll(self->fd, timeout_ms) < 0) ? Py_True : Py_False;
...
static PyObject *Serial_open2(SerialObject *self, PyObject *args, PyObject *kwds) {
    ...
    if ((self->fd = serial_open_advanced(path, baudrate, databits,
                                         parity, stopbits, xonxoff, rtscts))
        < 0) {
        PyErr_SetFromErrno(PyExc_IOError);
        return NULL;
    }
...
static PyObject *Serial_exit(SerialObject *self, PyObject *args) {
    PyObject *exc_type = 0;
...
static PyObject *Serial_str(PyObject *self) {
