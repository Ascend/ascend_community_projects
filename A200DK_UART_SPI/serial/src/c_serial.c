...
readn(int fd, void *vptr, size_t n)
{
	...
	ssize_t	nread;// read函数返回值
	char	*ptr; // 指向接下来在vptr中要写入的位置

	ptr = vptr;	  // 初始化
	nleft = n;	  // 初始化，需要读取n个字节
	while (nleft > 0) {
		if ( (nread = read(fd, ptr, nleft)) < 0) { // read出错，判断错误类型
			if (errno == EINTR)
				nread = 0;		// 将nread置0，继续调用read
			else
				return(-1);		// 其他错误，返回
		} else if (nread == 0)
			break;				// read返回0时表示EOF
		nleft -= nread;			// 更新剩余需要读取的字节数
		ptr   += nread;			// 更新下一次写入的位置
	}
...
{
	if (read_cnt <= 0) {	// static的read_cnt被定义时初始化为0
	again:					// 万恶的goto
		// 试图读取MAXLINE个字节
		if ( (read_cnt = read(fd, read_buf, sizeof(read_buf))) < 0) { 
			if (errno == EINTR)		// 熟悉的错误处理
				goto again;
			return(-1);
		} else if (read_cnt == 0)	// 熟悉的返回值0
			return(0);
		read_ptr = read_buf;		// 初始化read_ptr指针
	}

	read_cnt--;						// 被读取了一个字符，总字符数-1
	*ptr = *read_ptr++;				// ptr指向当前被读取的字符的位置
									// 之后read_ptr指向下一个被读取的字符位置
	return(1);
}
...
{
	ssize_t	n, rc;		// n用于计数，rc存储my_read返回值
	char	c, *ptr;	// c存储my_read读取的字符， ptr指向vptr中接下来要存储的位置

	ptr = vptr;			// 初始化
	for (n = 1; n < (ssize_t)maxlen; n++) { // 开始循环读取
        // if ( (rc = my_read(fd, &c)) == 1) { // 一次读一个
		if ( (rc = my_read(fd, &c)) == 1 ) { // 一次读一个
			*ptr++ = c;	// 将读取的字符依次存入vptr中
			if (c == '\n') 
				break;	// 读取到换行符，一行读取完毕，终止循环
            // printf("ni%cin\r\n",c);
		} else if (rc == 0) {
			*ptr = 0;	// 添加尾0
			return(n - 1);	// 返回实际读取的字节数n-1（最后一次没有读取到字符，故-1）
		} else
			return(-1);		// 出错返回
	}
