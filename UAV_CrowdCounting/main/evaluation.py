...
class Dataset:
    ...
    def __init__(self, image_root_path, image_format='.jpg'):
        ...
        for image in test_images:
            image_file = os.path.join(os.path.join(image_root_path, image + image_format))
            image_name = image_file.split("/")[-1]
            self.images.append(image_name)
            self.data.append(self.default_loader(image_file))
            self.key_points.append(self.get_keypoints(image_file.replace(self.image_format, ".npy")))
    ...
    def __getitem__(self, item):
        ...
        key_points = self.key_points[item]
    ...
    @classmethod
    def get_keypoints(self, npy):
        key_points = np.load(npy)
        return key_points
